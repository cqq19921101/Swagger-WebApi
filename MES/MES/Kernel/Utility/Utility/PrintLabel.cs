...
namespace Liteon.Mes.Utility
{
    ...
    public class PrintLabel
    {
        ...
        #region 保存文件
        /// <summary>
        /// 將從TABLE讀取的二進制數據保存成模板文件
        /// </summary>
        /// <param name="fileName">文件名，不含路徑</param>
        /// <param name="body">文檔內容二進制數組</param>
        public static void SaveLabelFileToLocal(string fileName,byte[] body)
        {
            try
            {
                if (body != null)
                {
                    string downloadFileName = System.Windows.Forms.Application.StartupPath + "\\" + fileName;
                    if (File.Exists(downloadFileName))
                    {
                        File.SetAttributes(downloadFileName, FileAttributes.Normal);
                        File.Delete(downloadFileName);
                    }
                    File.WriteAllBytes(downloadFileName, body);
                }
            }
            catch(Exception ex)
            {
        ...
        /// </summary>
        /// <param name="dt">文件名使用變量名FILENAME（例如123.txt），不含路徑，文件需放在程序目錄下，其它列的變量就是要替換的變量名稱，DataTable列頭以&lt;變量名&gt;為格式，替換txt文檔中相同的地方，若變量值是NA，則變量所在行不打印</param>
        ...
        public static void PrintByPort(DataTable dt, SerialPort serialPort)
        {
            ...
            StreamReader sr;
            string AllContent;
            string strFilePath = System.Windows.Forms.Application.StartupPath + "\\" + dt.Rows[0][0].ToString();
            if (!File.Exists(strFilePath))
            {
                throw new Exception("Can not find the templet:" + @strFilePath);
            }
        ...
        public static void PrintByLabFile(DataTable dt)
        {
            ...
            Document doc = null;
            CreateCodesoftInstance(strFilePath, ref ac, ref doc);
            try
            {
                PrintByLabFile(ac, doc, dt);
            }
            catch (Exception ex)
            {
                MessageBox.Show("打印出錯:" + ex.Message);
            }
            finally
            {
                ReleaseCodeSoftObj(ac, doc);
            }
        }
        ...
        private static List<string> GetVarList(ApplicationClass ac, Document doc)
        {
            try
            {
                ...
                {
                    varList.Add((string)keyValueArrays[i, 0]);
                    doc.Variables.FormVariables.Item((string)keyValueArrays[i, 0]).Value = "";//清空所有變量
                }
                vars = null;
                keyValueArrays = null;
                return varList;
            ...
            {
                //throw new Exception("獲取模板變量失敗:" + ex.Message);
                return null;
            }
            finally
            {
        ...
        private static void PrintLab(string defPrinter,ApplicationClass ac, Document doc, DataTable dt, 
            string fileName, List<string> varList)
        {
            try
            {
                //替換每一列的數據
                for (int i = 1; i < dt.Columns.Count; i++)
                {
                    ...
                    if (varList.Contains(colName))
                    {
                        doc.Variables.FormVariables.Item(colName).Value = dt.Rows[0][i].ToString();
                    }
                }
                //替換完成后打印
                bool bStatus = doc.Printer.SwitchTo(defPrinter, "", false);
                short sQty = doc.PrintDocument(1);
        ...
        /// 實例化Codesoft
        /// </summary>
        /// <param name="fileName">模板名稱，需要包含全路徑</param>
        /// <param name="ac">LabelManager2.ApplicationClass</param>
        /// <param name="doc">LabelManager2.Document</param>
        public static void CreateCodesoftInstance(string fileName,ref ApplicationClass ac,ref Document doc)
        {
            try
            {
                ReleaseCodeSoftObj(ac, doc);
                ac = new ApplicationClass();
                ac.Documents.Open(fileName, false);
                doc = ac.ActiveDocument;
            }
            catch(Exception ex)
            {
                throw new Exception("實例化codesoft失敗:" + ex.Message);
            }
        }
        ...
        /// <param name="ac">LabelManager2.ApplicationClass</param>
        /// <param name="doc">LabelManager2.Document</param>
        public static void ReleaseCodeSoftObj(ApplicationClass ac, Document doc)
        {
            try
            {
                if (doc != null)
